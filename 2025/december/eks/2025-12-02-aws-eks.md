...
```
┌─────────────────────────────────────────────────────────────────┐
│              CAMPUS MANAGEMENT (AWS Managed VPC)                │
│                                                                 │
│    ┌─────────────────────────────────────────────────────────┐ │
│    │              CAMPUS OPERATIONS                          │ │
│    │                (Control Plane)                          │ │
│    │                                                         │ │
│    │   📋 Reception Desk (API Server) - handles all requests│ │
│    │   🗄️ Filing Cabinets (etcd) - stores all records       │ │
│    │   📅 Room Assignment (Scheduler) - assigns workspace   │ │
│    │   👔 Supervisors (Controllers) - ensure things run     │ │
│    │                                                         │ │
│    └─────────────────────────────────────────────────────────┘ │
│                           │                                     │
└───────────────────────────┼─────────────────────────────────────┘
                            │
                   ☎️ Internal Intercom (ENIs)
...
│    │              BUILDING A (Subnet 1)                      │ │
│    │                                                         │ │
│    │   🚪 Room 1 (Node 1)        🚪 Room 2 (Node 2)          │ │
│    │   ┌─────────────────┐       ┌─────────────────┐        │ │
│    │   │ Desk 1 (Pod)    │       │ Desk 4 (Pod)    │        │ │
│    │   │ Desk 2 (Pod)    │       │ Desk 5 (Pod)    │        │ │
│    │   │ Desk 3 (Pod)    │       │ Desk 6 (Pod)    │        │ │
│    │   └─────────────────┘       └─────────────────┘        │ │
│    │                                                         │ │
│    │   Each room has a Room Manager (kubelet)               │ │
│    │   who reports to Campus Management                     │ │
│    └─────────────────────────────────────────────────────────┘ │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
- ✅ The building (Control Plane)
- ✅ Reception, security, utilities
- ✅ Communication systems

You handle:
- 👷 Hiring your team (provisioning nodes)
- 📋 Managing your team (node lifecycle)
- 💰 Paying your staff (EC2 costs)

### Full-Service Plan (EKS Auto Mode)

Building management provides:
- ✅ Everything in Standard, PLUS
- ✅ Staff provided for you (nodes managed by AWS)
- ✅ Automatic scaling of workforce

You only:
- 🎯 Define what work needs to be done (deploy pods)

```
┌─────────────────────────────┐   ┌─────────────────────────────┐
│      STANDARD PLAN          │   │     FULL-SERVICE PLAN       │
│                             │   │                             │
│  Building: ✅ Managed       │   │  Building: ✅ Managed       │
│  Your Team: 👷 You hire     │   │  Your Team: ✅ Provided     │
│  Your Work: 🎯 You do       │   │  Your Work: 🎯 You do       │
│                             │   │                             │
│  More control               │   │  Less hassle                │
│  More responsibility        │   │  More cost                  │
└─────────────────────────────┘   └─────────────────────────────┘
```
...
```
More AWS Control ◄──────────────────────────► More Your Control
     
┌──────────┬──────────┬───────────┬───────────┬──────────────┐
│  Auto    │  Fargate │  Managed  │ Karpenter │ Self-Managed │
│  Mode    │          │  Node Grp │           │  Nodes       │
└──────────┴──────────┴───────────┴───────────┴──────────────┘
     │          │           │           │            │
     ▼          ▼           ▼           ▼            ▼
...
```
PUBLIC ONLY:
  You (anywhere) ───► Public Phone ───► Campus Management ✅
  Room Manager ───► Public Phone ───► Campus Management ✅

PRIVATE ONLY:
  You (inside) ───► Intercom ───► Campus Management ✅
  You (outside) ───► ❌ Can't reach
  Room Manager ───► Intercom ───► Campus Management ✅

BOTH ENABLED:
  You (outside) ───► Public Phone ───► Campus Management ✅
  Room Manager ───► Intercom ───► Campus Management ✅ (preferred)
```
...
|---------------|------|---------||
| Room Manager → Campus Mgmt | 443 | Report status, get instructions |
| Campus Mgmt → Room Manager | 10250 | Check on desks, direct commands |
| Room Manager ↔ Room Manager | All | Cross-building coordination |
...
│                                                         │
│   ✅ Allowed:                                           │
│   📞 Room Manager → HQ (port 443): Status reports      │
│   📞 HQ → Room Manager (port 10250): kubectl exec/logs │
│   📞 Room Manager ↔ Room Manager: Cross-building comm  │
│                                                         │
│   ❌ Blocked:                                           │
│   🚫 Random desks calling HQ directly                  │
│   🚫 Outside calls without proper credentials          │
└─────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│               CAMPUS MANAGEMENT SETUP                       │
│                                                             │
│   Location A (AZ-a)    Location B (AZ-b)    Location C (AZ-c)
│   ┌─────────────┐      ┌─────────────┐      ┌─────────────┐ │
│   │ Reception   │      │ Reception   │      │ Reception   │ │
│   │ (API Server)│      │ (API Server)│      │ (API Server)│ │
│   │             │      │             │      │             │ │
│   │ Records 📁  │◄────►│ Records 📁  │◄────►│ Records 📁  │ │
│   │ (etcd copy) │ sync │ (etcd copy) │ sync │ (etcd copy) │ │
│   └─────────────┘      └─────────────┘      └─────────────┘ │
│                                                             │
│   If Location A goes down → B & C continue operating       │
...
- ✅ Integrated with AWS services (IAM, VPC, ELB, etc.)
...
8. **Cluster Security Group** - controls room ↔ campus management communication
