...
```
┌─────────────────────────────────────────────────────────────┐
│           WHY FLUENTBIT RUNS AS DAEMONSET                   │
│                                                             │
...
│  │                                                    │    │
│  │  Problems:                                        │    │
│  │  • Node 3 has no log collector!                   │    │
│  │  • Need to manually set replicas = node count    │    │
│  │  • New nodes won't get FluentBit automatically   │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
...
│  │  ✅ New nodes get FluentBit automatically        │    │
│  │  ✅ Access to node's /var/log/containers/        │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
### FluentBit Pipeline: Input → Filter → Output

```
┌─────────────────────────────────────────────────────────────┐
│              FLUENTBIT PIPELINE                             │
│                                                             │
│  Like a filing clerk's workflow:                           │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
...
│  │  │  • systemd: Read systemd journal         │     │    │
│  │  │  • tcp/udp: Receive over network         │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │                          │                         │    │
│  │                          ▼                         │    │
│  │  FILTER (Process & Enrich)                        │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │  🔧 "How do I process these logs?"        │     │    │
│  │  │                                          │     │    │
│  │  │  • kubernetes: Add pod name, namespace,  │     │    │
│  │  │    labels from K8s API                   │     │    │
│  │  │  • parser: Parse JSON/regex              │     │    │
...
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  Raw log: "ERROR: Connection timeout"                      │
│                    │                                        │
│                    ▼                                        │
│  FluentBit queries Kubernetes API Server                   │
│  "What pod wrote to this log file?"                        │
│                    │                                        │
│                    ▼                                        │
│  Enriched log:                                             │
│  {                                                         │
...
│  }                                                         │
│                                                             │
│  Now you can search: "Show all ERROR logs from production" │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │   🖥️ Big Screens in the Operations Center          │    │
│  │                                                    │    │
│  │   ┌─────────────┬─────────────┬─────────────┐     │    │
│  │   │ CPU Usage   │ Request/s   │ Error Rate  │     │    │
│  │   │ ┌───────┐   │ ┌───────┐   │ ┌───────┐   │     │    │
│  │   │ │ 📈    │   │ │ 📊    │   │ │ 📉    │   │     │    │
│  │   │ │  75%  │   │ │ 1.5K  │   │ │ 0.5%  │   │     │    │
│  │   │ └───────┘   │ └───────┘   │ └───────┘   │     │    │
...
│  │   │ WARN: High memory usage on pod-xyz      │     │    │
│  │   └─────────────────────────────────────────┘     │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Grafana itself doesn't store data!                        │
│  It QUERIES data from backends (data sources)              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│              GRAFANA DATA SOURCES                           │
│                                                             │
│  Grafana can query MULTIPLE backends in ONE dashboard:     │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │           ┌─────────────────┐                     │    │
...
│  │     ▼              ▼              ▼               │    │
│  │  ┌──────┐     ┌──────────┐    ┌───────┐          │    │
│  │  │Prome-│     │CloudWatch│    │ X-Ray │          │    │
│  │  │theus │     │  Logs    │    │Traces │          │    │
│  │  └──────┘     └──────────┘    └───────┘          │    │
│  │  Metrics       Logs           Traces              │    │
│  │                                                    │    │
│  │  One dashboard showing ALL three pillars! ✅      │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
...
│  • X-Ray (traces)                                          │
│  • MySQL, PostgreSQL (application data)                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  User views Grafana dashboard                              │
│         │                                                   │
│         ▼                                                   │
│  Grafana sends query to Prometheus                         │
│  "Give me HTTP requests per second for last 1 hour"        │
│         │                                                   │
│         ▼                                                   │
...
│         │                                                   │
│         ▼                                                   │
│  Prometheus returns data to Grafana                        │
│  [{time: 10:00, value: 150}, {time: 10:01, value: 155}...] │
│         │                                                   │
│         ▼                                                   │
│  Grafana renders the chart                                 │
│                                                             │
│  ⚠️ Grafana = Display layer                                │
...
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  Self-Managed Grafana          AWS Managed (AMG)           │
│  ┌────────────────────┐       ┌────────────────────┐      │
│  │                    │       │                    │      │
│  │ You manage:        │       │ AWS manages:       │      │
│  │ • EC2/EKS hosting  │       │ • Infrastructure   │      │
│  │ • High availability│  VS   │ • High availability│      │
│  │ • Upgrades         │       │ • Upgrades         │      │
│  │ • Security patches │       │ • Security patches │      │
│  │ • Backups          │       │ • Backups          │      │
│  │                    │       │                    │      │
│  │ ✅ Full control    │       │ ✅ Less work       │      │
│  │ ✅ No license cost │       │ ✅ AWS IAM login   │      │
│  │ 🔧 More work       │       │ 💰 Pay per user    │      │
│  │                    │       │                    │      │
│  └────────────────────┘       └────────────────────┘      │
│                                                             │
│  Both can connect to AWS services (CloudWatch, AMP, X-Ray) │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  │                                                    │    │
│  │   Temperature > 85°F?  → 🔔 HVAC Alert            │    │
│  │   Occupancy > 90%?     → 🔔 Capacity Alert        │    │
│  │   Power usage > 500kW? → 🔔 Energy Alert          │    │
│  │   Fire detected?       → 🔔🔔🔔 EVACUATE!         │    │
│  │                                                    │    │
│  │   EKS Examples:                                   │    │
│  │   CPU > 80%?           → 🔔 Scale up alert        │    │
│  │   Error rate > 5%?     → 🔔 On-call page          │    │
│  │   Pod restarts > 3?    → 🔔 Investigate           │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  │     "Everything is fine, CPU at 45%"              │    │
│  │                                                    │    │
│  │  🔴 ALARM                                          │    │
│  │     Metric breached threshold                     │    │
│  │     "CPU at 92%! Above 80% threshold!"            │    │
│  │     → Triggers notification actions               │    │
│  │                                                    │    │
│  │  ⚪ INSUFFICIENT_DATA                              │    │
│  │     Not enough data to evaluate                   │    │
│  │     "Alarm just created, waiting for data"        │    │
│  │     "Metric stopped reporting"                    │    │
...
│  ALARM → OK (recovered)                                    │
│  INSUFFICIENT_DATA → OK/ALARM (data arrives)               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│           CONFIGURING "CPU > 80% FOR 5 MINUTES"            │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │  Threshold:          80%                          │    │
...
│  │  Timeline:                                        │    │
│  │  ┌────┬────┬────┬────┬────┐                      │    │
│  │  │ 85%│ 82%│ 78%│ 88%│ 91%│ → NOT ALARM         │    │
│  │  └────┴────┴────┴────┴────┘   (only 4/5 above)  │    │
│  │                                                    │    │
│  │  ┌────┬────┬────┬────┬────┐                      │    │
│  │  │ 85%│ 82%│ 83%│ 88%│ 91%│ → ALARM! 🔴         │    │
│  │  └────┴────┴────┴────┴────┘   (5/5 above 80%)   │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  This prevents alerts from brief spikes!                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  │          ▼         ▼         ▼                    │    │
│  │       📧 Email  📱 SMS   💬 Slack                 │    │
│  │       (devops@) (on-call) (alerts channel)        │    │
│  │                                                    │    │
│  │  OR trigger automated actions:                    │    │
│  │                                                    │    │
│  │  • Auto Scaling: Add more instances               │    │
│  │  • Lambda: Run custom remediation                 │    │
│  │  • Systems Manager: Execute runbook               │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │  Regular Alarms (noisy):                          │    │
│  │  🔴 CPU > 80%    → Alert!                         │    │
│  │  🔴 Memory > 85% → Alert!                         │    │
│  │  🔴 Disk > 90%   → Alert!                         │    │
│  │                                                    │    │
│  │  3 separate alerts for maybe 1 real problem!      │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Composite Alarm (smarter):                                │
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │  Alert only if:                                   │    │
...
│  │  (Disk > 90%)                                     │    │
│  │                                                    │    │
│  │  Uses AND/OR logic to reduce false positives!     │    │
│  │                                                    │    │
│  │  Use cases:                                       │    │
│  │  • Alert if BOTH CPU and Memory are high         │    │
│  │  • Alert if ANY of 3 critical services are down  │    │
│  │  • Multi-region: Alert if BOTH regions have issues│    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
3. **FluentBit = Filing clerks** - DaemonSet, Input→Filter→Output pipeline, queries K8s API for metadata
