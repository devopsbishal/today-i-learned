# EKS Storage - The Locker & Storage Room System

> Understanding persistent storage in EKS using PV, PVC, StorageClass, EBS, and EFS - continuing the office campus analogy.

---

## TL;DR

| EKS Storage Concept | Office Campus Analogy |
|---------------------|----------------------|
| PersistentVolume (PV) | Actual locker or room assigned to you |
| PersistentVolumeClaim (PVC) | Request form: "I need a locker with X capacity" |
| StorageClass | Category in facilities catalog (basic locker, fireproof safe, shared room) |
| Static Provisioning | Facilities has empty lockers ready, assigns on request |
| Dynamic Provisioning | Facilities orders & installs new locker when you request |
| CSI Driver | Facilities team (installs lockers, cuts keys, moves lockers) |
| EBS Volume | Personal locker (one key, one worker - but locker can move) |
| EFS | Shared team room (multiple keys, whole team enters together) |
| ReadWriteOnce (RWO) | Personal locker - only you have the key |
| ReadWriteMany (RWX) | Shared room - everyone has a copy of the key |
| Volume mounting | Getting your key and accessing your locker/room |

---

## The Problem: Workers Need Persistent Storage

Workers (pods) are temporary - they come and go. But their documents need to survive!

```
WITHOUT PERSISTENT STORAGE:
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│   Worker starts → Creates documents → Worker leaves         │
│                                              ↓              │
│                                      Documents GONE! ❌     │
│                                                             │
└─────────────────────────────────────────────────────────────┘

WITH PERSISTENT STORAGE:
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│   Worker starts → Stores in locker → Worker leaves          │
│                                              ↓              │
│                              Documents SAFE in locker! ✅   │
│                                              ↓              │
│                              New worker → Same locker → Documents!
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## The Building Blocks

### PersistentVolume (PV) - The Actual Locker

A PV is the **actual storage** - like a physical locker in the building.

```
┌─────────────────────────────────────────────────────────────┐
│                    STORAGE ROOM                             │
│                                                             │
│   ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐      │
│   │ Locker  │  │ Locker  │  │ Locker  │  │ Locker  │      │
│   │   #1    │  │   #2    │  │   #3    │  │   #4    │      │
│   │  10GB   │  │  20GB   │  │  50GB   │  │  100GB  │      │
│   │ (free)  │  │ (used)  │  │ (free)  │  │ (used)  │      │
│   └─────────┘  └─────────┘  └─────────┘  └─────────┘      │
│                                                             │
│   Each locker = PersistentVolume                           │
│   Has specific size & type                                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### PersistentVolumeClaim (PVC) - The Request Form

A PVC is a **request** for storage - like filling out a form asking facilities for a locker.

```
┌─────────────────────────────────────────────────────────────┐
│                STORAGE REQUEST FORM (PVC)                   │
│                                                             │
│   Employee: payment-service                                 │
│   Department: production                                    │
│                                                             │
│   Requirements:                                             │
│   ┌─────────────────────────────────────────────────────┐  │
│   │ □ Locker type: fireproof (StorageClass: gp3)       │  │
│   │ □ Size needed: 10GB                                 │  │
│   │ □ Access: Just me (ReadWriteOnce)                   │  │
│   └─────────────────────────────────────────────────────┘  │
│                                                             │
│   Signature: ____________                                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

```yaml
# PVC in Kubernetes
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: payment-storage
  namespace: production
spec:
  storageClassName: gp3           # Locker type
  accessModes:
    - ReadWriteOnce               # Just me
  resources:
    requests:
      storage: 10Gi               # Size needed
```

---

### How PV and PVC Work Together

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  SEPARATION OF CONCERNS                                     │
│                                                             │
│  Platform Team (Admin)         Developer                    │
│  ┌───────────────────┐        ┌───────────────────┐        │
│  │ "I manage the     │        │ "I just need      │        │
│  │  storage backend" │        │  10GB storage"    │        │
│  │                   │        │                   │        │
│  │  Creates PVs or   │        │  Creates PVC      │        │
│  │  StorageClasses   │        │  with requirements│        │
│  └─────────┬─────────┘        └─────────┬─────────┘        │
│            │                            │                   │
│            └──────────┬─────────────────┘                   │
│                       ▼                                     │
│              ┌─────────────────┐                           │
│              │   KUBERNETES    │                           │
│              │                 │                           │
│              │ Matches PVC to  │                           │
│              │ suitable PV     │                           │
│              │                 │                           │
│              │ Status: Bound ✅│                           │
│              └─────────────────┘                           │
│                                                             │
│  Developer doesn't care HOW storage is provided!           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### StorageClass - The Facilities Catalog

StorageClass defines **types of storage** available - like a catalog of locker options.

```
┌─────────────────────────────────────────────────────────────┐
│                  FACILITIES STORAGE CATALOG                 │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ OPTION 1: Basic Locker (gp2)                        │   │
│  │ • Standard metal locker                             │   │
│  │ • Good for general documents                        │   │
│  │ • Cost: $                                           │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ OPTION 2: Premium Locker (gp3)                      │   │
│  │ • Faster access, better performance                 │   │
│  │ • Good for frequently accessed documents            │   │
│  │ • Cost: $$                                          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ OPTION 3: High-Security Safe (io2)                  │   │
│  │ • Ultra-fast access, maximum performance            │   │
│  │ • For critical, high-access documents               │   │
│  │ • Cost: $$$                                         │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ OPTION 4: Shared Team Room (efs)                    │   │
│  │ • Multiple workers can access together              │   │
│  │ • For shared team documents                         │   │
│  │ • Cost: $$                                          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

```yaml
# StorageClass in Kubernetes
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
  name: gp3
provisioner: ebs.csi.aws.com    # Who installs lockers
parameters:
  type: gp3                      # AWS EBS type
  encrypted: "true"              # Locked with encryption
reclaimPolicy: Delete            # What happens when PVC deleted
volumeBindingMode: WaitForFirstConsumer
```

---

## Static vs Dynamic Provisioning

### Static Provisioning - Pre-Built Lockers

Facilities has **empty lockers ready** and assigns when requested.

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  1. Admin pre-creates lockers (PVs)                        │
│     ┌─────────┐  ┌─────────┐  ┌─────────┐                 │
│     │ Locker  │  │ Locker  │  │ Locker  │                 │
│     │  10GB   │  │  20GB   │  │  50GB   │                 │
│     │ (empty) │  │ (empty) │  │ (empty) │                 │
│     └─────────┘  └─────────┘  └─────────┘                 │
│                                                             │
│  2. Developer requests 10GB (PVC)                          │
│                                                             │
│  3. K8s matches to existing 10GB locker                    │
│     ┌─────────┐                                            │
│     │ Locker  │ ← Assigned!                                │
│     │  10GB   │                                            │
│     │ (used)  │                                            │
│     └─────────┘                                            │
│                                                             │
│  Manual work: Admin must pre-create PVs                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

### Dynamic Provisioning - On-Demand Lockers

Facilities **orders and installs new locker** when you request.

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  1. Developer creates PVC                                  │
│     "I need a gp3 locker, 10GB"                           │
│                        │                                    │
│                        ▼                                    │
│  2. StorageClass sees request                              │
│     "gp3 class = I'll order from EBS supplier"            │
│                        │                                    │
│                        ▼                                    │
│  3. CSI Driver (facilities team):                          │
│     • Orders locker from AWS (CreateVolume)                │
│     • Installs it in the building                          │
│     • Creates PV automatically                             │
│                        │                                    │
│                        ▼                                    │
│  4. PVC bound to new PV                                    │
│     ┌─────────┐                                            │
│     │ Locker  │ ← Brand new, just for you!                 │
│     │  10GB   │                                            │
│     │ (bound) │                                            │
│     └─────────┘                                            │
│                                                             │
│  Automatic! No admin work needed!                          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## CSI Driver - The Facilities Team

**CSI = Container Storage Interface** - A standard for how Kubernetes talks to storage systems.

The CSI Driver is like the **facilities management team** that handles all storage operations.

```
┌─────────────────────────────────────────────────────────────────┐
│                      EBS CSI DRIVER                             │
│                   (Facilities Team)                             │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │           CSI CONTROLLER                                │   │
│  │           (Facilities Manager)                          │   │
│  │                                                         │   │
│  │  • Watches for new storage requests (PVCs)             │   │
│  │  • Orders lockers from AWS (CreateVolume API)          │   │
│  │  • Creates PV records in Kubernetes                    │   │
│  │  • Handles locker disposal when not needed             │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │           CSI NODE PLUGIN                               │   │
│  │           (Facilities Worker - on every node)           │   │
│  │                                                         │   │
│  │  • Installs locker outside your room (AttachVolume)    │   │
│  │  • Cuts the key (formats filesystem)                   │   │
│  │  • Gives you access (mounts into pod)                  │   │
│  │  • Removes locker when you leave (DetachVolume)        │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## The Complete Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                                                                 │
│  1. Developer creates PVC                                      │
│     ┌───────────────────────────────────────┐                  │
│     │ kind: PersistentVolumeClaim           │                  │
│     │ spec:                                 │                  │
│     │   storageClassName: gp3               │                  │
│     │   resources:                          │                  │
│     │     requests:                         │                  │
│     │       storage: 10Gi                   │                  │
│     └───────────────────────────────────────┘                  │
│                        │                                        │
│                        ▼                                        │
│  2. CSI Controller sees PVC                                    │
│     "gp3 StorageClass uses ebs.csi.aws.com provisioner"       │
│                        │                                        │
│                        ▼                                        │
│  3. CSI Controller calls AWS API                               │
│     CreateVolume(size=10GB, type=gp3, encrypted=true)         │
│                        │                                        │
│                        ▼                                        │
│  4. AWS creates EBS volume: vol-abc123                         │
│                        │                                        │
│                        ▼                                        │
│  5. CSI Controller creates PV in Kubernetes                    │
│     ┌───────────────────────────────────────┐                  │
│     │ kind: PersistentVolume                │                  │
│     │ spec:                                 │                  │
│     │   capacity: 10Gi                      │                  │
│     │   csi:                                │                  │
│     │     driver: ebs.csi.aws.com           │                  │
│     │     volumeHandle: vol-abc123          │                  │
│     └───────────────────────────────────────┘                  │
│                        │                                        │
│                        ▼                                        │
│  6. Kubernetes binds PVC to PV                                 │
│     PVC Status: Bound ✅                                       │
│                        │                                        │
│                        ▼                                        │
│  7. Pod created with PVC                                       │
│     ┌───────────────────────────────────────┐                  │
│     │ volumes:                              │                  │
│     │   - name: data                        │                  │
│     │     persistentVolumeClaim:            │                  │
│     │       claimName: my-pvc               │                  │
│     │ containers:                           │                  │
│     │   - volumeMounts:                     │                  │
│     │       - mountPath: /data              │                  │
│     │         name: data                    │                  │
│     └───────────────────────────────────────┘                  │
│                        │                                        │
│                        ▼                                        │
│  8. CSI Node Plugin on scheduled node:                         │
│     • Attaches vol-abc123 to EC2 instance                     │
│     • Formats filesystem (ext4)                                │
│     • Mounts to /var/lib/kubelet/pods/.../volumes/...         │
│                        │                                        │
│                        ▼                                        │
│  9. Pod can read/write to /data! 🎉                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## EBS vs EFS: Personal Locker vs Shared Room

### EBS - Personal Locker (ReadWriteOnce)

EBS is like a **personal locker** - only one key, follows you when you move.

```
┌─────────────────────────────────────────────────────────────┐
│                      EBS BEHAVIOR                           │
│                                                             │
│  Day 1: Worker in Room A (Node A)                          │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  Room A (Node A)                                     │  │
│  │  ┌─────────────┐    ┌─────────────┐                 │  │
│  │  │ Worker      │───▶│   Locker    │                 │  │
│  │  │ (Pod)       │    │   (EBS)     │                 │  │
│  │  └─────────────┘    │  10GB data  │                 │  │
│  │                     └─────────────┘                 │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  Day 2: Worker moves to Room B (Node B)                    │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  Room B (Node B)                                     │  │
│  │  ┌─────────────┐    ┌─────────────┐                 │  │
│  │  │ Worker      │───▶│   Locker    │ ← Same locker!  │  │
│  │  │ (Pod)       │    │   (EBS)     │   Moved here!   │  │
│  │  └─────────────┘    │  10GB data  │                 │  │
│  │                     └─────────────┘                 │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ✅ Data survives pod restarts and moves!                  │
│  ❌ Only ONE worker can use it at a time                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**CANNOT do this with EBS:**

```
❌ MULTIPLE WORKERS, DIFFERENT ROOMS:

Room A              Room B              Room C
┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│ Worker 1    │    │ Worker 2    │    │ Worker 3    │
│ writing...  │    │ writing...  │    │ writing...  │
└──────┬──────┘    └──────┬──────┘    └──────┬──────┘
       │                  │                  │
       └──────────────────┼──────────────────┘
                          │
                    ┌───────────┐
                    │  Locker   │
                    │   (EBS)   │
                    └───────────┘

EBS = Block storage = Can only attach to ONE node at a time!
```

---

### EFS - Shared Team Room (ReadWriteMany)

EFS is like a **shared team room** - everyone has a key, access from anywhere.

```
┌─────────────────────────────────────────────────────────────┐
│                      EFS BEHAVIOR                           │
│                                                             │
│  Room A (Node A)    Room B (Node B)    Room C (Node C)     │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐      │
│  │ Worker 1    │   │ Worker 2    │   │ Worker 3    │      │
│  │ reading/    │   │ reading/    │   │ reading/    │      │
│  │ writing     │   │ writing     │   │ writing     │      │
│  └──────┬──────┘   └──────┬──────┘   └──────┬──────┘      │
│         │                 │                 │              │
│         │           ┌─────┴─────┐           │              │
│         │           │           │           │              │
│         └───────────┤  SHARED   ├───────────┘              │
│                     │   ROOM    │                          │
│                     │   (EFS)   │                          │
│                     │           │                          │
│                     └───────────┘                          │
│                                                             │
│  ✅ Multiple workers from different rooms                  │
│  ✅ All see the same files                                 │
│  ✅ All can read AND write simultaneously                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Access Modes Explained

| Access Mode | Abbreviation | EBS | EFS | Meaning |
|-------------|--------------|-----|-----|---------|
| **ReadWriteOnce** | RWO | ✅ | ✅ | One node can mount read-write |
| **ReadOnlyMany** | ROX | ✅* | ✅ | Many nodes can mount read-only |
| **ReadWriteMany** | RWX | ❌ | ✅ | Many nodes can mount read-write |

*EBS ROX requires io2 with Multi-Attach feature

---

## EBS vs EFS Comparison

| Aspect | EBS (Personal Locker) | EFS (Shared Room) |
|--------|----------------------|-------------------|
| **Access Mode** | ReadWriteOnce | ReadWriteMany |
| **Multiple pods** | Same node only | Any node ✅ |
| **Performance** | Higher (16,000 IOPS) | Lower (network FS) |
| **Latency** | Lower (block storage) | Higher (NFS protocol) |
| **Cost** | $0.08/GB/month (gp3) | $0.30/GB/month |
| **Use case** | Databases, single pod | Shared files, multi-pod |
| **AZ bound** | Yes (single AZ) | No (multi-AZ) |

---

## When to Use What

### Use EBS For:

```
┌─────────────────────────────────────────────────────────────┐
│                    EBS USE CASES                            │
│                                                             │
│  ✅ Databases (PostgreSQL, MySQL, MongoDB)                 │
│     └─ Single pod, high IOPS needed                        │
│                                                             │
│  ✅ Elasticsearch / Kafka / Redis                          │
│     └─ Each instance needs dedicated fast storage          │
│                                                             │
│  ✅ Jenkins master                                         │
│     └─ Single pod, persistent workspace                    │
│                                                             │
│  ✅ Any single-pod stateful application                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Use EFS For:

```
┌─────────────────────────────────────────────────────────────┐
│                    EFS USE CASES                            │
│                                                             │
│  ✅ WordPress / CMS (shared media uploads)                 │
│     └─ Multiple frontend pods share /wp-content/uploads    │
│                                                             │
│  ✅ Shared configuration files                             │
│     └─ All pods read same config                           │
│                                                             │
│  ✅ ML model storage                                       │
│     └─ Multiple inference pods read same model             │
│                                                             │
│  ✅ Shared logs / reports                                  │
│     └─ Multiple pods write to shared location              │
│                                                             │
│  ✅ Static website assets                                  │
│     └─ Multiple web servers serve same files               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Decision Flowchart

```
Need persistent storage for pods?
│
├── How many pods need access simultaneously?
│   │
│   ├── ONE pod at a time
│   │   │
│   │   └── Need high performance?
│   │       ├── YES → EBS gp3 or io2
│   │       └── NO  → EBS gp3
│   │
│   └── MULTIPLE pods simultaneously
│       │
│       └── EFS ✅
│           (Only option for ReadWriteMany)

Quick Rule:
┌─────────────────────────────────────────────────────────────┐
│  Single pod, high performance → EBS                        │
│  Multiple pods, shared access → EFS                        │
└─────────────────────────────────────────────────────────────┘
```

---

## Real-World Example: WordPress

```
┌─────────────────────────────────────────────────────────────────┐
│                    WORDPRESS DEPLOYMENT                         │
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │                   DATABASE TIER                         │  │
│   │                                                         │  │
│   │   StatefulSet: mysql (1 replica)                       │  │
│   │   ┌─────────────┐                                      │  │
│   │   │   MySQL     │───▶ EBS gp3 20GB                     │  │
│   │   │    Pod      │     (ReadWriteOnce)                  │  │
│   │   └─────────────┘     High IOPS for DB writes         │  │
│   │                                                         │  │
│   └─────────────────────────────────────────────────────────┘  │
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │                   WEB TIER                              │  │
│   │                                                         │  │
│   │   Deployment: wordpress (3 replicas)                   │  │
│   │                                                         │  │
│   │   Node A          Node B          Node C               │  │
│   │   ┌─────────┐    ┌─────────┐    ┌─────────┐           │  │
│   │   │WordPress│    │WordPress│    │WordPress│           │  │
│   │   │  Pod 1  │    │  Pod 2  │    │  Pod 3  │           │  │
│   │   └────┬────┘    └────┬────┘    └────┬────┘           │  │
│   │        │              │              │                 │  │
│   │        └──────────────┼──────────────┘                 │  │
│   │                       │                                 │  │
│   │                  ┌────────┐                            │  │
│   │                  │  EFS   │ ← /wp-content/uploads      │  │
│   │                  │ 50GB   │   (ReadWriteMany)          │  │
│   │                  └────────┘   All pods see uploads     │  │
│   │                                                         │  │
│   └─────────────────────────────────────────────────────────┘  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## Installing CSI Drivers in EKS

### EBS CSI Driver

```bash
# Install via EKS add-on (recommended)
aws eks create-addon \
  --cluster-name my-cluster \
  --addon-name aws-ebs-csi-driver \
  --service-account-role-arn arn:aws:iam::123:role/EBS-CSI-Role

# Or via Helm
helm install aws-ebs-csi-driver aws-ebs-csi-driver/aws-ebs-csi-driver \
  --namespace kube-system
```

### EFS CSI Driver

```bash
# Install via EKS add-on (recommended)
aws eks create-addon \
  --cluster-name my-cluster \
  --addon-name aws-efs-csi-driver \
  --service-account-role-arn arn:aws:iam::123:role/EFS-CSI-Role

# Or via Helm
helm install aws-efs-csi-driver aws-efs-csi-driver/aws-efs-csi-driver \
  --namespace kube-system
```

**Note:** CSI drivers need IAM permissions (use IRSA or Pod Identity!)

---

## Key Takeaways

1. **PV = actual storage, PVC = request for storage** - Separation of concerns
2. **StorageClass enables dynamic provisioning** - No manual PV creation
3. **CSI Driver = facilities team** - Handles all storage operations
4. **EBS = personal locker (RWO)** - One pod at a time, high performance
5. **EFS = shared room (RWX)** - Multiple pods, any node
6. **EBS is faster but single-pod** - Use for databases
7. **EFS is shared but slower** - Use for shared files

---

## Related Reading

- [AWS VPC - The Compound Analogy](../november/2025-11-30-aws-vpc.md)
- [AWS EKS - The Managed Office Building Analogy](./2025-12-02-aws-eks.md)
- [AWS VPC CNI - The Phone Extension System](./2025-12-03-aws-vpc-cni.md)
- [EKS Networking - The Mail & Delivery System](./2025-12-09-eks-networking.md)
- [EKS IAM - The Employee Badge System](./2025-12-10-eks-iam-irsa-pod-identity.md)

---

*Written on December 11, 2025*
