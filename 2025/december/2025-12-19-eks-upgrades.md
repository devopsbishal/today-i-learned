# EKS Upgrades & Maintenance - The Building Renovation

> Understanding EKS cluster upgrades, node update strategies, and version management through control plane and data plane upgrades - continuing the office campus analogy.

---

## TL;DR

| EKS Upgrades Concept | Office Campus Analogy |
|---------------------|----------------------|
| Kubernetes Version Upgrade | Building modernization (upgrading facilities) |
| Control Plane Upgrade | Management office upgrade (no tenant impact) |
| Worker Node Upgrade | Office floor renovation (where employees work) |
| Rolling Update | Renovate one floor at a time, move people temporarily |
| Blue/Green Deployment | Build new wing, move everyone, demolish old |
| Version Skew Policy (n-2) | Old floors can be 2 generations behind main building |
| 14-month Standard Support | Free building maintenance period |
| 12-month Extended Support | Pay premium ($0.60/hr) for outdated systems |
| Auto-upgrade after EOL | City forces building code compliance |
| Cannot Skip Versions | Must upgrade step-by-step (2020→2022→2024) |
| EKS Addons | Building utilities (plumbing, HVAC, electrical) |
| Addon Compatibility | Utilities must match building standards |
| Pod Disruption Budget | Minimum staff on duty during renovation |
| Node Drain | Evacuate floor before renovation |
| Cordon | Close floor to new occupants |
| Pre-upgrade Testing | Test renovation in building's test wing |

---

## The Upgrade Challenge

Your office campus (cluster) needs regular modernization to stay current:

```
┌─────────────────────────────────────────────────────────────┐
│              WHY UPGRADE YOUR BUILDING?                     │
│                                                             │
│  🔒 Security                                                │
│     Old building systems have known vulnerabilities        │
│                                                             │
│  🆕 New Features                                            │
│     Modern amenities, better efficiency                     │
│                                                             │
│  🐛 Bug Fixes                                               │
│     Fixed elevator issues, HVAC improvements                │
│                                                             │
│  📅 Compliance                                              │
│     Meet current building codes                            │
│                                                             │
│  💰 Support Costs                                           │
│     Outdated systems cost more to maintain                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Version Lifecycle - Building Age Policy

```
┌─────────────────────────────────────────────────────────────┐
│           KUBERNETES VERSION LIFECYCLE                      │
│                                                             │
│  MONTH 0: New Version Released (e.g., K8s 1.30)           │
│  ┌────────────────────────────────────────────────────┐    │
│  │  🆕 Latest & Greatest                              │    │
│  │     All new features, fully supported              │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  MONTH 1-14: Standard Support (FREE)                       │
│  ┌────────────────────────────────────────────────────┐    │
│  │  ✅ Full AWS Support                               │    │
│  │  ✅ Security patches                               │    │
│  │  ✅ Bug fixes                                      │    │
│  │  💰 No extra cost                                  │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  MONTH 15-26: Extended Support ($0.60/cluster-hour)        │
│  ┌────────────────────────────────────────────────────┐    │
│  │  ⚠️ Limited support                                │    │
│  │  ✅ Critical security patches only                 │    │
│  │  ❌ No new features                                │    │
│  │  💰 $0.60/hour = ~$432/month per cluster           │    │
│  │                                                    │    │
│  │  Use case: Need time to test upgrades             │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  MONTH 27+: End of Life - AUTO UPGRADE!                    │
│  ┌────────────────────────────────────────────────────┐    │
│  │  🚨 AWS FORCE UPGRADES YOUR CLUSTER!              │    │
│  │     1.28 → 1.29 (next oldest supported version)   │    │
│  │                                                    │    │
│  │  ⚠️ This happens automatically!                   │    │
│  │  ⚠️ Could break your applications!                │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Best Practice: Upgrade every 3-6 months during standard  │
│  support period!                                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Version Skew Policy - Building Compatibility Rules

```
┌─────────────────────────────────────────────────────────────┐
│           VERSION SKEW POLICY (n-2)                         │
│                                                             │
│  Control Plane: K8s 1.30                                   │
│  ┌────────────────────────────────────────────────────┐    │
│  │  🏢 Management Building (Modern - 2024)            │    │
│  │     API Server, Scheduler, Controller Manager      │    │
│  └────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          │ n-2 skew allowed                │
│                          ▼                                  │
│  Worker Nodes: Can be 1.28, 1.29, or 1.30                 │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Floor A   Floor B   Floor C                       │    │
│  │  (2022)    (2023)    (2024)                        │    │
│  │  K8s 1.28  K8s 1.29  K8s 1.30                     │    │
│  │  ✅ OK     ✅ OK     ✅ OK                         │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  ❌ INVALID: Control Plane 1.30, Nodes 1.27               │
│     3 versions behind = violates n-2 policy!               │
│                                                             │
│  ✅ VALID: Control Plane 1.30, Nodes 1.28                 │
│     2 versions behind = within n-2 policy                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Why n-2 Matters

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  Allows gradual upgrades without forcing all at once:      │
│                                                             │
│  Day 1:  Control Plane 1.28 → 1.29                        │
│          Nodes stay at 1.28 (still compatible!)           │
│                                                             │
│  Week 2: Gradually upgrade node groups                     │
│          Nodes 1.28 → 1.29                                 │
│                                                             │
│  Month 2: Control Plane 1.29 → 1.30                       │
│           Nodes stay at 1.29 (still compatible!)          │
│                                                             │
│  ✅ Zero pressure to upgrade everything immediately!      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## The Upgrade Sequence - Renovation Order

```
┌─────────────────────────────────────────────────────────────┐
│              MANDATORY UPGRADE ORDER                        │
│                                                             │
│  STEP 1: Control Plane (Management Office)                 │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Upgrade: 1.28 → 1.29                             │    │
│  │  Time: 20-30 minutes                               │    │
│  │  Downtime: ZERO for applications                   │    │
│  │  AWS handles: Rolling update of API servers        │    │
│  └────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          ▼                                  │
│  STEP 2: EKS Addons (Building Utilities)                   │
│  ┌────────────────────────────────────────────────────┐    │
│  │  VPC CNI:    1.15.x → 1.16.x                      │    │
│  │  CoreDNS:    1.10.x → 1.11.x                      │    │
│  │  kube-proxy: 1.28.x → 1.29.x                      │    │
│  │                                                    │    │
│  │  Check compatibility matrix first!                │    │
│  └────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          ▼                                  │
│  STEP 3: Worker Nodes (Office Floors)                      │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Choose strategy:                                  │    │
│  │  • Rolling Update (slow, safe, cheaper)           │    │
│  │  • Blue/Green (fast, flexible, costly)            │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  ⚠️ CRITICAL: Cannot skip steps!                           │
│  ⚠️ CRITICAL: Cannot skip versions (1.28 → 1.30)!         │
│     Must go 1.28 → 1.29 → 1.30                            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Control Plane Upgrade - Management Office Renovation

AWS handles control plane upgrades with **zero downtime** for your applications.

```
┌─────────────────────────────────────────────────────────────┐
│         CONTROL PLANE UPGRADE (AWS Managed)                 │
│                                                             │
│  Before: K8s 1.28                                          │
│  ┌────────────────────────────────────────────────────┐    │
│  │  API Server 1   API Server 2   API Server 3       │    │
│  │  (1.28)        (1.28)          (1.28)             │    │
│  │  Scheduler      Controller Mgr  etcd               │    │
│  │  (1.28)        (1.28)          (1.28)             │    │
│  └────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          │ AWS performs rolling update     │
│                          ▼                                  │
│  During Upgrade: Mixed versions temporarily                │
│  ┌────────────────────────────────────────────────────┐    │
│  │  API Server 1   API Server 2   API Server 3       │    │
│  │  (1.29) ✅     (1.28)          (1.28)             │    │
│  │                                                    │    │
│  │  One at a time, health checked before next        │    │
│  └────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          ▼                                  │
│  After: K8s 1.29                                           │
│  ┌────────────────────────────────────────────────────┐    │
│  │  API Server 1   API Server 2   API Server 3       │    │
│  │  (1.29) ✅     (1.29) ✅       (1.29) ✅          │    │
│  │  Scheduler      Controller Mgr  etcd               │    │
│  │  (1.29) ✅     (1.29) ✅       (1.29) ✅          │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  ✅ Applications keep running on worker nodes!            │
│  ✅ API requests handled throughout upgrade!              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### How to Upgrade Control Plane

```bash
# Via AWS CLI
aws eks update-cluster-version \
  --name my-cluster \
  --kubernetes-version 1.29

# Via AWS Console
# EKS → Clusters → my-cluster → Update now

# Monitor upgrade status
aws eks describe-update \
  --name my-cluster \
  --update-id <update-id>
```

---

## Worker Node Upgrades - Floor Renovation

Two main strategies: **Rolling Update** vs **Blue/Green**

### Strategy 1: Rolling Update - One Floor at a Time

```
┌─────────────────────────────────────────────────────────────┐
│              ROLLING UPDATE STRATEGY                        │
│                                                             │
│  Initial State: 4 nodes on K8s 1.28                        │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1    Node 2    Node 3    Node 4             │    │
│  │  (1.28)    (1.28)    (1.28)    (1.28)             │    │
│  │  🏢🏢🏢    🏢🏢      🏢🏢      🏢                │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Step 1: Cordon Node 1 (close to new tenants)             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1    Node 2    Node 3    Node 4             │    │
│  │  🚫(1.28)  (1.28)    (1.28)    (1.28)             │    │
│  │            🏢🏢      🏢🏢      🏢                │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Step 2: Drain Node 1 (evacuate tenants)                  │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1    Node 2    Node 3    Node 4             │    │
│  │  🚫(1.28)  (1.28)    (1.28)    (1.28)             │    │
│  │  [EMPTY]   🏢🏢🏢🏢  🏢🏢🏢    🏢🏢              │    │
│  │            ↑ Pods moved here                       │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Step 3: Upgrade Node 1 → 1.29                            │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1    Node 2    Node 3    Node 4             │    │
│  │  ✅(1.29)  (1.28)    (1.28)    (1.28)             │    │
│  │  🏢        🏢🏢🏢🏢  🏢🏢🏢    🏢🏢              │    │
│  │  ↑ Uncordoned, accepting pods again                │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Repeat for Node 2, 3, 4...                                │
│                                                             │
│  Final State: All nodes on 1.29                            │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1    Node 2    Node 3    Node 4             │    │
│  │  (1.29)    (1.29)    (1.29)    (1.29)             │    │
│  │  🏢🏢      🏢🏢🏢    🏢🏢      🏢                │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  ✅ Pros: Cost effective, no extra resources               │
│  ⚠️ Cons: Slow (one at a time), temporary capacity loss   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Rolling Update Configuration

```yaml
# Managed Node Group Update Config
updateConfig:
  maxUnavailable: 1        # Max nodes unavailable at once
  # OR
  maxUnavailablePercentage: 33  # Max 33% unavailable
```

```bash
# Update managed node group
aws eks update-nodegroup-version \
  --cluster-name my-cluster \
  --nodegroup-name my-nodes \
  --kubernetes-version 1.29
```

---

### Strategy 2: Blue/Green - Build New Wing

```
┌─────────────────────────────────────────────────────────────┐
│            BLUE/GREEN DEPLOYMENT STRATEGY                   │
│                                                             │
│  PHASE 1: Current State (BLUE)                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Blue Node Group (K8s 1.28)                       │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ Node 1  Node 2  Node 3  Node 4           │     │    │
│  │  │ 🏢🏢   🏢🏢    🏢🏢    🏢             │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │                                                    │    │
│  │  100% of traffic here                             │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  PHASE 2: Create GREEN node group                          │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Blue Node Group (K8s 1.28)                       │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ Node 1  Node 2  Node 3  Node 4           │     │    │
│  │  │ 🏢🏢   🏢🏢    🏢🏢    🏢             │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │  100% traffic                                      │    │
│  │                                                    │    │
│  │  Green Node Group (K8s 1.29) - NEW!               │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ Node 5  Node 6  Node 7  Node 8           │     │    │
│  │  │ [EMPTY] [EMPTY] [EMPTY] [EMPTY]          │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │  0% traffic (ready but not used)                  │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  PHASE 3: Cordon BLUE (stop new pods)                     │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Blue Node Group (CORDONED)                       │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ 🚫      🚫      🚫      🚫               │     │    │
│  │  │ 🏢🏢   🏢🏢    🏢🏢    🏢             │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │                                                    │    │
│  │  Green Node Group                                  │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ Node 5  Node 6  Node 7  Node 8           │     │    │
│  │  │ 🏢      🏢      🏢      🏢               │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │  ↑ New pods go here!                              │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  PHASE 4: Gradually drain BLUE nodes                       │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Blue: 30% traffic → 0% traffic (over hours)      │    │
│  │  Green: 70% traffic → 100% traffic                │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  PHASE 5: Delete BLUE node group                           │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Green Node Group (K8s 1.29)                      │    │
│  │  ┌──────────────────────────────────────────┐     │    │
│  │  │ Node 5  Node 6  Node 7  Node 8           │     │    │
│  │  │ 🏢🏢   🏢🏢    🏢🏢    🏢             │     │    │
│  │  └──────────────────────────────────────────┘     │    │
│  │  100% traffic                                      │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  ✅ Pros: Fast, easy rollback, gradual traffic shift      │
│  ⚠️ Cons: 2x resources during transition (costly!)        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Blue/Green Steps

```bash
# 1. Create GREEN node group with new version
aws eks create-nodegroup \
  --cluster-name my-cluster \
  --nodegroup-name green-nodes \
  --kubernetes-version 1.29 \
  --subnets subnet-xxx subnet-yyy \
  --node-role arn:aws:iam::123:role/NodeRole

# 2. Wait for GREEN nodes to be ready
kubectl get nodes

# 3. Cordon BLUE nodes (stop new pods)
kubectl cordon -l nodegroup=blue-nodes

# 4. Gradually drain BLUE nodes
kubectl drain <blue-node-name> --ignore-daemonsets --delete-emptydir-data

# 5. Verify apps on GREEN nodes
kubectl get pods -o wide

# 6. Delete BLUE node group
aws eks delete-nodegroup \
  --cluster-name my-cluster \
  --nodegroup-name blue-nodes
```

---

## Rolling vs Blue/Green Decision Guide

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  Use ROLLING UPDATE when:                                  │
│  ✅ Cost-sensitive (no 2x capacity needed)                 │
│  ✅ Can tolerate gradual rollout (hours)                   │
│  ✅ Have good Pod Disruption Budgets                       │
│  ✅ Low-risk upgrade (minor version bump)                  │
│  ✅ Dev/staging environments                               │
│                                                             │
│  Use BLUE/GREEN when:                                      │
│  ✅ Need fast rollback capability                          │
│  ✅ Zero-downtime critical (e-commerce, finance)           │
│  ✅ Want to test new version in production traffic         │
│  ✅ Major version upgrade (higher risk)                    │
│  ✅ Can afford 2x capacity temporarily                     │
│  ✅ Production environments                                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## Pod Disruption Budgets - Safety During Renovation

PDBs ensure **minimum availability** during node drains.

```
┌─────────────────────────────────────────────────────────────┐
│          POD DISRUPTION BUDGET (PDB)                        │
│                                                             │
│  Scenario: 5 replicas, PDB minAvailable: 4                │
│                                                             │
│  Before Drain:                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1      Node 2      Node 3                    │    │
│  │  🏢🏢       🏢🏢       🏢                         │    │
│  │  Pod 1,2    Pod 3,4    Pod 5                       │    │
│  │  5 pods running ✅                                 │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Drain Node 1 Attempt:                                     │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Kubernetes: "Can I evict Pod 1?"                  │    │
│  │  PDB: "Check: 5 running - 1 evict = 4 remaining"  │    │
│  │        "4 >= minAvailable(4)? YES! ✅"            │    │
│  │        "ALLOW eviction"                            │    │
│  │                                                    │    │
│  │  Kubernetes: "Can I evict Pod 2?"                  │    │
│  │  PDB: "Check: 4 running - 1 evict = 3 remaining"  │    │
│  │        "3 >= minAvailable(4)? NO! ❌"             │    │
│  │        "BLOCK eviction - wait for Pod 1 to start" │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  After Pod 1 reschedules on Node 2:                       │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Node 1      Node 2      Node 3                    │    │
│  │  🏢          🏢🏢🏢     🏢                         │    │
│  │  Pod 2      Pod 1,3,4  Pod 5                       │    │
│  │  Now can evict Pod 2! (5-1=4 >= minAvailable)     │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  PDB protects application availability! 🛡️                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### PDB Example

```yaml
apiVersion: policy/v1
kind: PodDisruptionBudget
metadata:
  name: api-server-pdb
spec:
  minAvailable: 4        # At least 4 pods must be available
  # OR
  # maxUnavailable: 1    # Max 1 pod can be down at a time
  selector:
    matchLabels:
      app: api-server
```

```yaml
# Alternative: percentage-based
apiVersion: policy/v1
kind: PodDisruptionBudget
metadata:
  name: worker-pdb
spec:
  maxUnavailable: 25%    # Max 25% can be down
  selector:
    matchLabels:
      app: background-worker
```

---

## EKS Addons - Building Utilities Upgrade

Three critical addons need version compatibility checks:

```
┌─────────────────────────────────────────────────────────────┐
│              EKS ADDONS (Building Utilities)                │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │                                                    │    │
│  │  🔌 VPC CNI (Networking Plumbing)                 │    │
│  │     Gives pods IP addresses                       │    │
│  │     Version must match K8s version                │    │
│  │                                                    │    │
│  │  🌐 CoreDNS (Building Directory)                  │    │
│  │     DNS resolution for services                   │    │
│  │     Version must match K8s version                │    │
│  │                                                    │    │
│  │  🔀 kube-proxy (Network Routing)                  │    │
│  │     Routes traffic to pods                        │    │
│  │     Version matches K8s version                   │    │
│  │                                                    │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Compatibility Matrix Example:                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  K8s 1.29:                                         │    │
│  │  • VPC CNI: 1.16.x - 1.18.x                       │    │
│  │  • CoreDNS: 1.11.x                                 │    │
│  │  • kube-proxy: 1.29.x                              │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Check compatibility: https://docs.aws.amazon.com/eks/     │
│  latest/userguide/managing-add-ons.html                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Updating Addons

```bash
# List current addon versions
aws eks describe-addon-versions \
  --kubernetes-version 1.29

# Update VPC CNI addon
aws eks update-addon \
  --cluster-name my-cluster \
  --addon-name vpc-cni \
  --addon-version v1.16.0-eksbuild.1 \
  --resolve-conflicts OVERWRITE

# Update CoreDNS addon
aws eks update-addon \
  --cluster-name my-cluster \
  --addon-name coredns \
  --addon-version v1.11.1-eksbuild.4

# Update kube-proxy addon
aws eks update-addon \
  --cluster-name my-cluster \
  --addon-name kube-proxy \
  --addon-version v1.29.0-eksbuild.1
```

---

## Pre-Upgrade Checklist - Testing in Test Wing

```
┌─────────────────────────────────────────────────────────────┐
│              PRE-UPGRADE CHECKLIST                          │
│                                                             │
│  ☐ 1. Review Release Notes                                 │
│     Check breaking changes, deprecated APIs                │
│     https://kubernetes.io/docs/setup/release/notes/        │
│                                                             │
│  ☐ 2. Test in Non-Production First                         │
│     Dev → Staging → Production                             │
│     Never upgrade production first!                        │
│                                                             │
│  ☐ 3. Check Deprecated APIs                                │
│     kubectl get --raw /metrics | grep deprecated           │
│     Use: https://github.com/doitintl/kube-no-trouble       │
│                                                             │
│  ☐ 4. Verify Addon Compatibility                           │
│     Check VPC CNI, CoreDNS, kube-proxy versions            │
│                                                             │
│  ☐ 5. Review Application Dependencies                      │
│     Check if apps use deprecated K8s APIs                  │
│                                                             │
│  ☐ 6. Create Pod Disruption Budgets                        │
│     Ensure apps have PDBs for critical services            │
│                                                             │
│  ☐ 7. Backup Critical Data                                 │
│     etcd backup, application data snapshots                │
│                                                             │
│  ☐ 8. Plan Maintenance Window                              │
│     Inform stakeholders, schedule low-traffic time         │
│                                                             │
│  ☐ 9. Prepare Rollback Plan                                │
│     How to revert if issues occur?                         │
│                                                             │
│  ☐ 10. Monitor Post-Upgrade                                │
│      Watch logs, metrics, alerts for 24-48 hours           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Detecting Deprecated APIs

```bash
# Install kube-no-trouble (kubent)
kubectl krew install deprecations

# Check for deprecated API usage
kubectl deprecations

# Example output:
# API: extensions/v1beta1/Ingress
# DEPRECATED IN: v1.14
# REMOVED IN: v1.22
# FILE: ingress.yaml
```

---

## Common Deprecated APIs

| API Version | Replaced By | Removed In |
|-------------|-------------|------------|
| `extensions/v1beta1/Ingress` | `networking.k8s.io/v1/Ingress` | 1.22 |
| `rbac.authorization.k8s.io/v1beta1` | `rbac.authorization.k8s.io/v1` | 1.22 |
| `apiextensions.k8s.io/v1beta1` | `apiextensions.k8s.io/v1` | 1.22 |
| `batch/v1beta1/CronJob` | `batch/v1/CronJob` | 1.25 |
| `policy/v1beta1/PodDisruptionBudget` | `policy/v1/PodDisruptionBudget` | 1.25 |
| `autoscaling/v2beta2/HPA` | `autoscaling/v2/HPA` | 1.26 |

---

## The Complete Upgrade Flow

```
┌─────────────────────────────────────────────────────────────────┐
│              COMPLETE UPGRADE WORKFLOW                          │
│                                                                 │
│  Week 1: Preparation                                           │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │  • Read release notes                                     │ │
│  │  • Check deprecated APIs                                  │ │
│  │  • Verify addon compatibility                             │ │
│  │  • Create/update Pod Disruption Budgets                   │ │
│  └───────────────────────────────────────────────────────────┘ │
│                          │                                      │
│                          ▼                                      │
│  Week 2: Dev Cluster Upgrade                                   │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │  1. Upgrade control plane (1.28 → 1.29)                  │ │
│  │  2. Update addons (VPC CNI, CoreDNS, kube-proxy)         │ │
│  │  3. Rolling update worker nodes                           │ │
│  │  4. Test applications thoroughly                          │ │
│  │  5. Document any issues found                             │ │
│  └───────────────────────────────────────────────────────────┘ │
│                          │                                      │
│                          ▼                                      │
│  Week 3: Staging Cluster Upgrade                               │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │  Repeat same steps as Dev                                 │ │
│  │  Run integration tests, load tests                        │ │
│  │  Monitor for 48 hours                                     │ │
│  └───────────────────────────────────────────────────────────┘ │
│                          │                                      │
│                          ▼                                      │
│  Week 4: Production Upgrade                                    │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │  BEFORE:                                                  │ │
│  │  • Schedule maintenance window (low traffic)              │ │
│  │  • Notify stakeholders                                    │ │
│  │  • Prepare rollback plan                                  │ │
│  │                                                           │ │
│  │  DURING:                                                  │ │
│  │  1. Upgrade control plane → 20-30 min                    │ │
│  │  2. Update addons → 10-15 min                            │ │
│  │  3. Blue/Green node upgrade → 2-4 hours                  │ │
│  │     OR Rolling update → 4-8 hours                        │ │
│  │                                                           │ │
│  │  AFTER:                                                   │ │
│  │  • Monitor metrics, logs, alerts (24-48 hours)           │ │
│  │  • Check application health                              │ │
│  │  • Document upgrade completion                           │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## Quick Reference

| Component | Upgrade Time | Downtime | Can Skip Versions? |
|-----------|-------------|----------|-------------------|
| Control Plane | 20-30 min | Zero | ❌ No (1.28→1.29→1.30) |
| EKS Addons | 10-15 min | Zero | ❌ No |
| Nodes (Rolling) | 4-8 hours | Per-node | ❌ No |
| Nodes (Blue/Green) | 2-4 hours | Zero | ❌ No |

---

## Common Pitfalls

| Problem | Solution |
|---------|----------|
| **Forgot to check deprecated APIs** | Use kubent/kube-no-trouble before upgrade |
| **No Pod Disruption Budgets** | Create PDBs for all critical services |
| **Upgraded production first** | Always upgrade dev → staging → prod |
| **Skipped versions (1.28→1.30)** | Must go one version at a time |
| **Addons incompatible** | Check compatibility matrix first |
| **Node drain stuck** | Check for pods without PDB, DaemonSets |
| **Didn't test in non-prod** | Test thoroughly in dev/staging first |

---

## Key Takeaways

1. **Version Support**: 14 months free, 12 months extended ($0.60/hr), then auto-upgrade
2. **Version Skew (n-2)**: Nodes can be up to 2 minor versions behind control plane
3. **Cannot Skip Versions**: Must upgrade 1.28 → 1.29 → 1.30 (one at a time)
4. **Upgrade Order**: Control Plane → Addons → Worker Nodes
5. **Control Plane**: Zero downtime, AWS manages rolling update
6. **Rolling Update**: Cost-effective, slower, one node at a time
7. **Blue/Green**: Fast, easy rollback, but 2x resources during transition
8. **Pod Disruption Budgets**: Critical for maintaining availability during drain
9. **Test First**: Always upgrade dev → staging → production
10. **Check Deprecated APIs**: Use kubent to find deprecated API usage before upgrade

---

## Related Reading

- [AWS VPC - The Compound Analogy](../november/2025-11-30-aws-vpc.md)
- [AWS EKS - The Managed Office Building Analogy](./2025-12-02-aws-eks.md)
- [AWS VPC CNI - The Phone Extension System](./2025-12-03-aws-vpc-cni.md)
- [EKS Networking - The Mail & Delivery System](./2025-12-09-eks-networking.md)
- [EKS IAM - The Employee Badge System](./2025-12-10-eks-iam-irsa-pod-identity.md)
- [EKS Storage - The Locker & Storage Room System](./2025-12-11-eks-storage.md)
- [EKS Autoscaling - The Staffing & Workspace Management](./2025-12-12-eks-autoscaling.md)
- [EKS Security - The Corporate Security System](./2025-12-14-eks-security.md)
- [EKS Observability - The Building Operations Center](./2025-12-16-eks-observability.md)

---

*Written on December 19, 2025*
