...
Inside this compound, we have multiple **blocks** (typically 3 per AWS region, some regions have up to 6). These are **Availability Zones (AZs)** - physically separate data centers with independent power, cooling, and networking. If one block catches fire, the others keep running.
...
```
Instance A crashes → detach EIP → attach to Instance B
              (Parking spot #52 now assigned to different car)
```

**Cost**: Since February 2024, AWS charges ~$0.005/hour ($3.65/month) for every public IPv4 address, including EIPs attached to running instances. Unused EIPs cost the same. This change encourages IPv6 adoption and careful public IP planning.
...
Private EC2 (10.0.1.50)
       │
       ▼
Route Table (Private) ──► "Send 0.0.0.0/0 to NAT Gateway"
       │
       ▼
NAT Gateway ──► Translates 10.0.1.50 → 52.xx.xx.xx
       │
       ▼
Route Table (Public) ──► "Send 0.0.0.0/0 to Internet Gateway"
       │
       ▼
...
```
Home Office ──►[VPN Tunnel]──► Virtual Private Gateway ──► VPC
...
4. Route traffic: 10.0.0.0/16 (AWS) ↔ 192.168.0.0/16 (home office)
...
```
Home Office ──► Direct Connect Location ──► AWS Region ──► VPC
              (physical fiber, 1/10/100 Gbps)           
```

**How it works:**
1. Order a **dedicated connection** (1, 10, or 100 Gbps) at an AWS Direct Connect location
2. AWS provisions their port at the DX location; you (or a partner) provision a **cross-connect** from your router to the AWS port
3. Optionally, use a partner to extend the connection from the DX location to your premises
...
│  ┌────────────────────────────────────────┐ │
│  │           NACL (Building Guard)        │ │
│  │  "Can this person enter the building?" │ │
│  └────────────────────────────────────────┘ │
│                     │                       │
│                     ▼                       │
│  ┌─────────────────────────────────────┐   │
│  │  Apartment (EC2)                    │   │
│  │  ┌───────────────────────────────┐  │   │
│  │  │ Security Group (Apt Guard)    │  │   │
│  │  │ "Can this person enter apt?"  │  │   │
│  │  └───────────────────────────────┘  │   │
│  └─────────────────────────────────────┘   │
└─────────────────────────────────────────────┘
```
...
```
Compound A ◄────tunnel────► Compound B
```

**The catch**: Tunnels are **not transitive**. If A connects to B, and B connects to C, A cannot talk to C through B. You need a separate A↔C tunnel.

For 3 VPCs: 3 tunnels
For 5 VPCs: 10 tunnels  
For N VPCs: N(N-1)/2 tunnels -- this grows fast
...
Now:
- A→Station→C works automatically (**transitive**)
...
```
Private Building → NAT Gateway → Internet Gateway → Internet → AWS S3
```
...
```
Private EC2 → Route Table → Gateway Endpoint → AWS S3
...
```
Private EC2 → Interface Endpoint (ENI: 10.0.1.55) → AWS Secrets Manager
...
- **Routing policies**:
  ...
  - **Geolocation**: European users → EU compound, US users → US compound
...
```
┌─────────────┐         ┌──────────────┐         ┌─────────────┐
│  Customer   │ ──1──►  │ Tokyo Edge   │ ──2──►  │  Virginia   │
│   (Tokyo)   │ ◄──4──  │  Location    │ ◄──3──  │  (Origin)   │
└─────────────┘         └──────────────┘         └─────────────┘
        ▲                      │                         
        │                      │                         
        └──────5 (cached)──────┘                         
...
Customer → ISP1 → ISP2 → ISP3 → ISP4 → AWS → Your VPC
  (15 hops, 150ms, packet loss)

Global Accelerator Route (optimized):
Customer → Nearest AWS Edge → AWS Backbone → Your VPC
